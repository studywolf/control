...
def Task(arm_class, control_type, x_bias=0., y_bias=.25, dist=.15):
    ...
    if issubclass(control_type, dmp.Shell):
    ...
    elif issubclass(control_type, lqr.Control):
        controller = lqr.Control()
        ...
        controller = osc.Control(kp=kp, kv=np.sqrt(kp), null_control=True)
        ...
        raise Exception('System must use DMP, LQR, or OSC control'\
                        '(dmp | lqr | osc) for reaching tasks.')
    ...
    if issubclass(arm_class, Arm3):
        runner_pars.update({'box':[-2.5,2.5,0,4]})
