...
class Control(control.Control):
    ...
    def __init__(self, solve_continuous=False, adaptation=None, **kwargs): 
        ...
        if self.write_to_file is True:
            ...
            # set up recorders
            self.u_recorder = Recorder('control signal', self.task, 'lqr_spsa')
            self.xy_recorder = Recorder('end-effector position', self.task, 'lqr_spsa')
            self.dist_recorder = Recorder('distance from target', self.task, 'lqr_spsa')
    ...
    def calc_derivs(self, x, u):
        ...
        for ii in range(max_iters):
            ...
            delta_j = ((state_inc - state_dec) / (2.0 * eps)).reshape(-1)
